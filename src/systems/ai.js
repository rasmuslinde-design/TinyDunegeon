...
function stepToward(map, from, to, enemies) {
  ...
  for (const m of moves) {
    ...
    const nx = from.x + m.x;
    const ny = from.y + m.y;
    if (!isSolid(map, nx, ny) && !enemies.some(e => e.alive && e.id !== from.id && e.x === nx && e.y === ny)) {
      return { x: nx, y: ny };
    }
  }
  return null;
...
function randomStep(map, from, enemies) {
...
export function tickAI(now) {
  ...
  const { levelData, enemies, player, damagePlayer, killEnemy, updateEnemy, spawnProjectile } = store;
  if (!levelData) return;

  enemies.forEach(enemy => {
    if (!enemy.alive) return;
    const base    = ENEMY_SPRITES[enemy.type] ?? { speed: 1000, dmg: 5, behavior: 'wander', hp: 20 };
    ...
    switch (base.behavior) {
      case 'wander':
        newPos = d <= 7 ? stepToward(levelData.map, enemy, player, enemies)
                       : randomStep(levelData.map, enemy, enemies);
        break;
      ...
      case 'erratic': {
        ...
        const nx = enemy.x + r.x, ny = enemy.y + r.y;
        newPos = d <= 5
          ? stepToward(levelData.map, enemy, player, enemies)
          : (!isSolid(levelData.map, nx, ny) ? { x: nx, y: ny } : null);
        break;
      }

      case 'stalk':
        if (d <= 9) newPos = stepToward(levelData.map, enemy, player, enemies);
        break;
      ...
      case 'charge':
        if (d <= 12) newPos = stepToward(levelData.map, enemy, player, enemies);
        break;

      case 'ranged': {
        // Keep 3-5 tiles away, shoot projectiles
        if (d > 5)      newPos = stepToward(levelData.map, enemy, player, enemies);
        else if (d < 3) {
          const flee = { x: enemy.x - sign(player.x - enemy.x), y: enemy.y - sign(player.y - enemy.y) };
          if (!isSolid(levelData.map, flee.x, flee.y)) newPos = flee;
        }
      ...
      case 'guard':
        if (d <= 4) newPos = stepToward(levelData.map, enemy, player, enemies);
        break;

      case 'boss': {
        newPos = stepToward(levelData.map, enemy, player, enemies);
    ...
    if (newPos) {
      updateEnemy(enemy.id, {
        ...
        aiTimer: now,
        facing: newPos.x > enemy.x ? 'right' : newPos.x < enemy.x ? 'left'
               : newPos.y > enemy.y ? 'down' : 'up',
      });

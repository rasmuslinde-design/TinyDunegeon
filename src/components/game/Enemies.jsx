// Enemy renderer with health bars + smooth pixel-lerp movement
...
function EnemySprite({ enemy }) {
  const [px, setPx] = useState(enemy.x * RENDERED_TILE);
  const [py, setPy] = useState(enemy.y * RENDERED_TILE);
  const targetX = useRef(enemy.x * RENDERED_TILE);
  const targetY = useRef(enemy.y * RENDERED_TILE);
  const rafRef = useRef(null);

  useEffect(() => {
    targetX.current = enemy.x * RENDERED_TILE;
    targetY.current = enemy.y * RENDERED_TILE;
  }, [enemy.x, enemy.y]);

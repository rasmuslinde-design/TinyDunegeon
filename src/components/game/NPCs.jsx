  ...
  return (
    <>
      {npcs.map(npc => {
        ...
        const near = Math.abs(npc.x - playerX) <= 1 && Math.abs(npc.y - playerY) <= 1;
        ...
        return (
          <React.Fragment key={npc.id}>
            ...
            <div style={{
              position: 'absolute',
              left: px - 4, top: py + RENDERED_TILE - 10,
              width: RENDERED_TILE + 8, height: 14,
              background: 'radial-gradient(ellipse, rgba(250,200,50,0.35) 0%, transparent 70%)',
              borderRadius: '50%',
            }} />

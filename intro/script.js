...
function createBench(x, z, rot, parent) {
    ...
    seat.setAttribute('width', '3'); seat.setAttribute('height', '0.1'); seat.setAttribute('depth', '1');
    seat.setAttribute('color', '#4a2c10'); seat.setAttribute('position', '0 0.5 0');
    const back = document.createElement('a-box');
    back.setAttribute('width', '3'); back.setAttribute('height', '0.6'); back.setAttribute('depth', '0.1');
    back.setAttribute('color', '#3d240d'); back.setAttribute('position', '0 0.9 -0.45');
    for(let i of [-1.2, 1.2]) {
        const leg = document.createElement('a-box');
        leg.setAttribute('width', '0.15'); leg.setAttribute('height', '0.5'); leg.setAttribute('depth', '0.8');
        leg.setAttribute('color', '#111'); leg.setAttribute('position', `${i} 0.25 0`);
        bench.appendChild(leg);
    }
    bench.appendChild(seat); bench.appendChild(back); parent.appendChild(bench);
}
...
function generateStars(num, parent) {
    for (let i = 0; i < num; i++) {
        ...
        star.setAttribute('material', { color: '#fff', shader: 'flat', emissive: '#fff', emissiveIntensity: 5 });
...
window.onload = () => {
    ...
    if (container) {
        ...
        for (let i = 0; i < 70; i++) {
            ...
            createObj('#tree-m', randX, 4, randZ, 1.5 + Math.random(), Math.random() * 360, container);
        }
    }
    ...
    setInterval(() => {
        ...
        canInteract = dist < 20; 
        document.getElementById('ui').style.display = canInteract ? 'block' : 'none';
...
function enterCastle() {
    ...
    setTimeout(() => {
        // Siia võid panna ka uue asukoha (teleporti lossi sisse)
        // Näiteks: cam.setAttribute('position', '0 1.6 -100');
